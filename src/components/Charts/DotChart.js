...
const parseDomain = () => {
  return [0, Math.max.apply(null, [...data01.map(entry => entry.value), ...data02.map(entry => entry.value)])]
}
...
class DotChart extends Component {
  ...
  render() {
    ...
    return (
      <Card>
        <CardHeader title="PageView" />
        <CardMedia style={{ padding: '1rem' }}>
          ...
          <ScatterChart width={800} height={60} margin={{ top: 10, right: 0, bottom: 0, left: 10 }}>
            <XAxis
              type="category"
              dataKey="hour"
              name="hour"
